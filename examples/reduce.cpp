...
decltype(auto) reduce_min(const Data& data, thp::threadpool& tp) {
  thp::algos::partitioner::equal_size algo(10000u, data.cbegin(), data.cend());
  thp::stl_algo::api tp_algo(tp);

  auto f = tp_algo.transform_reduce(data.cbegin(), data.cend(),
...
int main(int argc, const char* const argv[])
{
  ...
  try {
    ...
    if (use_stl)
    {
      ...
      cu.now();
      smin = std::transform_reduce( std::execution::seq,
                                      data.cbegin(), data.cend(),
                                      std::numeric_limits<int>::max(),
                                      [](auto&& a, auto&& b) { return std::min(a, b); },
                                      std::identity()
